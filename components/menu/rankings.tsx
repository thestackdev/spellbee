import { getPoints } from "@/components/index";
...
export default function Rankings(props: RankingsProps) {
  ...
  const [answersSum, setAnswersSum] = useState<number>(0);
  ...
  return (
    <MenuPage>
      ...
      <div className="p-4 font-semithin text-lg">
        <p>
          Below are the minimum scores for the rankings of the current puzzle.
        </p>
        <p>
          {" "}
          Please refer to <i>How to Play</i> for scoring system.
        </p>
      </div>
        ...
        {rankingLevels.map((i) => (
          <li key={i.name}>
            ...
            <span className="font-medium">
              {Math.floor(answersSum * i.minScoreMultiplier)}
            </span>
            )
          </li>
        ))}
