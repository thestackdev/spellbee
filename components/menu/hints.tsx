    ...
    <MenuPage>
      ...
      <div className="p-2">
        <p className="text-lg font-thin">
          The current puzzle has {answers.length} words.
        </p>
        {pangrams.length === 1 ? (
          <p className="text-lg font-thin">
      ...
      <div className="how-to-section pb-3">
        <p className="font-semithin">
          ...
          <span className="font-medium">
            {answers.filter((i) => i.length === 4).length}
          </span>
        </p>
        <p className="font-semithin">
          Five letters:{" "}
          <span className="font-medium">
            {answers.filter((i) => i.length === 5).length}
          </span>
        </p>
        <p className="font-semithin">
          Six letters:{" "}
          <span className="font-medium">
            {answers.filter((i) => i.length === 6).length}
          </span>
        </p>
        <p className="font-semithin">
          Seven letters:{" "}
          <span className="font-medium">
            {answers.filter((i) => i.length === 7).length}
          </span>
        </p>
        <p className="font-semithin">
          Eight or more letters:{" "}
          <span className="font-medium">
            {answers.filter((i) => i.length >= 8).length}
          </span>
        </p>
      </div>
      <hr />
      ...
      <div className="how-to-section flex flex-col">
        <p className="font-semithin">
          Reveal the answer list for the current game.
        </p>
        <button
          className="self-center single-btn border-0 m-1 text-bold bg-yellow-300 hover:bg-yellow-200 active:bg-black active:text-yellow-300"
          onClick={() => reveal()}
        >
          Reveal
        </button>
